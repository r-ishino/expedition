...
| 2      | ストリーム出力の構造化表示   | 中     | Claude Code出力をブロック種別（thinking/tool_use/text）ごとに折りたたみ表示。詳細: [stream_collapse_design.md](stream_collapse_design.md)                                  | Phase 1-2 完了、表示改善（ステータス・要約・集約）完了 |
| 2.5    | MCP サーバー（Waypoint操作） | 中     | stdio MCP サーバーで Waypoint CRUD ツールを公開。Claude Code が対話中に直接 Waypoint を操作可能に。詳細: [mcp_server_architecture.md](privates/mcp_server_architecture.md) | 実装完了（未検証）                                     |
